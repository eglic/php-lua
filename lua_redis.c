...
static int push_new_const( lua_State *L , const char *name , size_t name_len , int type ) {
...
static int lstatus_index( lua_State *L ) {
	...
	push_new_const( L , key , key_len , REDIS_REPLY_STATUS /* status */
	);
...
static redisContext* check_connection( lua_State *L , int idx ) {
	luahiredis_Connection *pConn = (luahiredis_Connection*) luaL_checkudata( L , idx , LUAHIREDIS_CONN_MT );
	if ( pConn == NULL ) {
	...
	if ( pConn->pContext == NULL ) {
		luaL_error( L , "lua-hiredis error: attempted to use closed connection" );
		return NULL; /* Unreachable */
	}
...
static int push_error( lua_State *L , redisContext *pContext ) {
	...
	lua_pushnil( L );
	lua_pushstring( L , ( pContext->errstr != NULL ) ? pContext->errstr : "(lua-hiredis: no error message)" /* TODO: ?! */
	);
...
static int load_args( lua_State *L , redisContext *pContext , int idx , /* index of first argument */
...
static int lconn_command( lua_State *L ) {
	...
	redisReply *pReply = (redisReply*) redisCommandArgv( pContext , nargs , argv , argvlen );
	if ( pReply == NULL ) {
...
static int lconn_close( lua_State *L ) {
	luahiredis_Connection *pConn = (luahiredis_Connection*) luaL_checkudata( L , 1 , LUAHIREDIS_CONN_MT );
...
static int lhiredis_connect( lua_State *L ) {
	...
	pResult = (luahiredis_Connection*) lua_newuserdata( L , sizeof(luahiredis_Connection) );
