...
ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_invoke, 0, 0, 1) ZEND_ARG_INFO(0, arg)
ZEND_ARG_INFO(0, ...)
ZEND_END_ARG_INFO()
/* }}} */
...
PHP_METHOD(lua_closure, invoke) {
	zval *this = getThis( );
	lua_closure_object *self = php_lua_closure_object_from_zend_object( Z_OBJ_P( this ) );
	int bp , sp;
	zval *arguments = NULL;
	lua_State *L = NULL;
	zval rv;
	...
	if ( Z_TYPE(self->lua) != IS_OBJECT || !instanceof_function( Z_OBJCE( self->lua ) , lua_ce ) ) {
		zend_throw_exception_ex( NULL , 0 , "corrupted Lua object" );
		return;
	}

	L = ( Z_LUAVAL( self->lua ) )->L;

	bp = lua_gettop( L );
	lua_rawgeti( L , LUA_REGISTRYINDEX , self->closure );
	...
	if ( lua_pcall(L, ZEND_NUM_ARGS(), LUA_MULTRET, 0) != 0 ) {
		...
		zend_throw_exception_ex( NULL , 0 , "call to lua function %s failed" , lua_tostring( L , -1 ) );
		return;
	}
	...
	if ( !sp ) {
		...
		php_lua_get_zval_from_lua( L , -1 , Z_OBJ( self->lua ) , return_value );
	} else {
		zval rv;
		int i = 0;
		array_init( return_value );
		for ( i = -sp ; i < 0 ; i++ ) {
			php_lua_get_zval_from_lua( L , i , Z_OBJ( self->lua ) , &rv );
...
zend_function_entry lua_closure_methods[] =
	{
	PHP_ME(lua_closure, __construct, arginfo_lua_ctor, ZEND_ACC_PRIVATE|ZEND_ACC_CTOR)
...
static void php_lua_closure_free_obj( zend_object *zobj ) /* {{{ */
{
	...
	if ( ( Z_TYPE(objval->lua) == IS_OBJECT ) && instanceof_function( Z_OBJCE( objval->lua ) , lua_ce ) ) {
