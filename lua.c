...
ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_call, 0, 0, 2) ZEND_ARG_INFO(0, method)
ZEND_ARG_INFO(0, args)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_assign, 0, 0, 2) ZEND_ARG_INFO(0, name)
ZEND_ARG_INFO(0, value)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_register, 0, 0, 2) ZEND_ARG_INFO(0, name)
ZEND_ARG_INFO(0, function)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_include, 0, 0, 1) ZEND_ARG_INFO(0, file)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_eval, 0, 0, 1) ZEND_ARG_INFO(0, statements)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_lua_free, 0, 0, 1) ZEND_ARG_INFO(0, closure)
ZEND_END_ARG_INFO()
/* }}} */
...
static int php_lua_md5( lua_State *L ) {
	...
	for ( i = 1 ; i <= nargs ; ++i ) {
		php_lua_get_zval_from_lua( L , i , NULL , &rv );
		if ( Z_TYPE(rv) != IS_STRING || Z_STRVAL(rv) < 1 ) {
			lua_pushnil( L );
			return 1;
		}
...
zval* php_lua_get_zval_from_lua( lua_State *L , int index , zend_object *lua_obj , zval *rv ) /* {{{ */{
	switch ( lua_type( L , index ) ) {
		...
		default:
			php_error_docref( NULL , E_WARNING , "unsupported type '%s' for php" , lua_typename( L , lua_type( L , index ) ) );
			ZVAL_NULL( rv );
			return NULL;
	}
	return rv;
...
static zval* php_lua_call_lua_function( zend_object *lua_obj , zval *func , zval *args , int use_self , zval *retval ) /* {{{ */{
	int bp = 0;
	int sp = 0;
	int arg_num = 0;
	zval rv;
	...
	} else if ( IS_OBJECT == Z_TYPE_P( func ) && instanceof_function( Z_OBJCE_P( func ) , php_lua_get_closure_ce() ) ) {
	...
	if ( lua_pcall(L, arg_num, LUA_MULTRET, 0) != LUA_OK ) {
		php_error_docref( NULL , E_WARNING , "call to lua function %s failed" , lua_tostring( L , -1 ) );
...
PHP_METHOD(lua, eval) {
	zval *this = getThis( );
	zend_object *self = this->value.obj;
	lua_State *L;
	char *statements;
	long bp , len;
	int ret;
	...
	if ( ( ret = luaL_loadbuffer( L , statements , len , "line" ) ) != LUA_OK || ( ret = lua_pcall(L, 0, LUA_MULTRET, 0) != LUA_OK ) ) {
		...
		if ( ret_count > 1 ) {
			...
			for ( i = -ret_count ; i < 0 ; i++ ) {
				php_lua_get_zval_from_lua( L , i , self , &rv );
				add_next_index_zval( return_value , &rv );
			}
		} else if ( ret_count ) {
			php_lua_get_zval_from_lua( L , -1 , self , return_value );
		}
		lua_pop( L , ret_count );
	}
}
...
PHP_METHOD(lua, call) {
	...
	if ( !( php_lua_call_lua_function( self , func , args , u_self , return_value ) ) ) {
		RETURN_FALSE
		;
	}
}
